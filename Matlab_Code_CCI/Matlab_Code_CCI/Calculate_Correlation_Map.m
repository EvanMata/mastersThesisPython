...
%fullfile(pwd,'Topography_Holograms_Reference_Filtered'); 

%baseFolder.Data     = fullfile(pwd,'Difference_Holograms_Reference_Filtered');  %Input Folder of reference-filtered diff holograms
%baseFolder.Topo     = fullfile(pwd,'Topography_Holograms_Reference_Filtered');  %Input Folder of Topo holograms
...
% EVERYTHING AFTER HERE (xept ROI?) IS CORRELATION BASED
...
Correlation = eye(length(len),length(len));
...
for ii = 1:(length(len)-1)
    ...
    fprintf(1, 'Calculating pair-correlations of holo: %d [%d,%d]; Finished: %0.3f %% \n', index_1,len(1),len(end),Status);
    
    %Varies second holo and uses symmetrie of correlation function
    for j = (ii+1):length(len)
        index_2 = len(j);
        %Pair-correlation function
        [Correlation(ii,j),~] = CCI_Correlation_Function(Data(index_1).Holo_Flattened(ROI,ROI),...
                                                        Data(index_2).Holo_Flattened(ROI,ROI),...
                                                        Data(index_1).Noise_Mask(ROI,ROI),...
                                                        Data(index_2).Noise_Mask(ROI,ROI));
    end
    
    %Use Symmetry
    Correlation((ii+1):end,ii) = transpose(Correlation(ii,(ii+1):end));
end

disp('Calculation of 2-time correlation matrix finished!')

%% Plot correlation map

figure; imagesc(Correlation,[-0.2 0.75]); axis image; axis xy
